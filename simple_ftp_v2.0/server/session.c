...
void session_table_init()
{
    ...
    for( int i = 0; i < SESSION_BUCKET_NUM; i++){
        buckets[i].bucket_num = 0;
        INIT_HLIST_NODE(&buckets[i].head);
...
{
    g_session_table = (session_table _t*)malloc(sizeof(session_table_t*) * SESSION_TABLE_NUM);
    if(!g_session_table){
        printf("%s line %d %d: malloc failed\n", __FILE__, __LINE__, __FUNCTION__);
        exit(1);
    }
...
{   
    ...
    flow_info.sip = ntohl(addr->sip);
    flow_info.sport = ntohs(addr->sport);
